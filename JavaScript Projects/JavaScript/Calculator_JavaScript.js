...
function Input_Decimal(dot) {
	...
	if (!Calculator.Display_Value.includes(dot)) {
		//we are saying that if the Display_Value does not contain a decimal point, we want
...
function Handle_Operator(Next_Operator) {
	...
	//when an operator key is pressed, we convert the current number displayed on the 
	//screen to a number and then store the result in Calculator. First_Operand if it
	//already exist.
	const Value_of_Input = parseFloat(Display_Value);
	//checks if an operator already exists and if Wait_Second_Operand is true, then updates 
